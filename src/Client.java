...
public class Client {
    public static void main(String[] args) throws InterruptedException, IOException {
        try(Socket socket = new Socket("localhost", 28553);// запускаем подключение сокета
            ...
            SocketChannel channel = SocketChannel.open(new InetSocketAddress("localhost", 28553));
            OutputStream outStream = Channels.newOutputStream(channel);
            ObjectOutputStream outToClient = new ObjectOutputStream(outStream);)
        {BufferedReader br =new BufferedReader(new InputStreamReader(System.in));
            ...
            while(!socket.isOutputShutdown()|| br.ready()) {
                    ...
                    InputStream get = Channels.newInputStream(channel);
                    ObjectInputStream inn = new ObjectInputStream(get);
